(function () {
    ...
    const getFilteredDomains = () => {
        ...
        return domainCatalog.filter((item) => {
            const matchesSearch = !browserState.search || item.name.toLowerCase().includes(browserState.search) || item.description.toLowerCase().includes(browserState.search);
    ...
    const renderDomainBrowser = () => {
        ...
        if (refs.domainLoadMore) {
            refs.domainLoadMore.hidden = filtered.length <= browserState.visibleCount;
        }
    ...
    const setupDomainBrowser = () => {
        ...
        window.addEventListener("resize", () => {
            if (browserState.visibleCount < getBrowserBatchSize()) {
                browserState.visibleCount = getBrowserBatchSize();
                renderDomainBrowser();
            }
        });
